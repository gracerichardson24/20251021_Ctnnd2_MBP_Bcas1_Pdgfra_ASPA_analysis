...
summary_combined_detections_ASPAPdgra <- combined_detections_ASPAPdgra %>%
  mutate(
    prop_Pdgfra_pos_ASPA_pos = Pdgfra_pos_ASPA_pos / Total_cells,
    prop_Pdgfra_pos_ASPA_neg = Pdgfra_pos_ASPA_neg / Total_cells,
    prop_Pdgfra_neg_ASPA_pos = Pdgfra_neg_ASPA_pos / Total_cells,
    prop_Pdgfra_neg_ASPA_neg = Pdgfra_neg_ASPA_neg / Total_cells
  )
...
```{<- summary_combined_detections_ASPAPdgra %>%}
  mutate(
    prop_Pdgfra_pos_ASPA_pos_trans = asin(sqrt(prop_Pdgfra_pos_ASPA_pos)),
    prop_Pdgfra_pos_ASPA_neg_trans = asin(sqrt(prop_Pdgfra_pos_ASPA_neg)),
    prop_Pdgfra_neg_ASPA_pos_trans = asin(sqrt(prop_Pdgfra_neg_ASPA_pos)),
    prop_Pdgfra_neg_ASPA_neg_trans = asin(sqrt(prop_Pdgfra_neg_ASPA_neg))
  )

t.test(prop_Pdgfra_pos_ASPA_pos_trans ~ Genotype, data = summary_combined_detections_ASPAPdgra)
t.test(prop_Pdgfra_pos_ASPA_neg_trans ~ Genotype, data = summary_combined_detections_ASPAPdgra)
t.test(prop_Pdgfra_neg_ASPA_pos_trans ~ Genotype, data = summary_combined_detections_ASPAPdgra)
t.test(prop_Pdgfra_neg_ASPA_neg_trans ~ Genotype, data = summary_combined_detections_ASPAPdgra)
```

```{r}
combined_ASPAPdgra_clean <- combined_ASPAPdgra_clean %>%
  rename(ASPA_total = `ASPA/total`)

summary_stats <- combined_ASPAPdgra_clean %>%
  group_by(Genotype) %>%
  summarise(
    MeanCount = mean(ASPA_total, na.rm = TRUE),
    SEM = sd(ASPA_total, na.rm = TRUE)/sqrt(n()),
    .groups = 'drop'
  )

ggplot(combined_ASPAPdgra_clean, aes(x = Genotype, y = ASPA_total)) +
