...
imageframe_combined_detections_ASPAPdgra <- combined_detections_ASPAPdgra %>%
  group_by(Animal, Genotype, Slice, Hemisphere) %>%
  summarise(
    Total_cells = n(),
    # Totals of the flag columns (they sum 0/1 -> counts)
...
```
```{r}
animal_frame_ASPAPdgra <- imageframe_combined_detections_ASPAPdgra %>%
...
summary_combined_detections_ASPAPdgra$Genotype <- factor(summary_combined_detections_ASPAPdgra$Genotype, levels = c("WT", "KO"))
...
ggplot(summary_combined_detections_ASPAPdgra, aes(x = Genotype, y = ASPA_per_area)) +
  ...
  geom_jitter (data = Imageframe_data, aes(color = Genotype, y = ASPA_per_area), width = 0.2, size = 2, alpha = 0.2) +
  theme_minimal() +
  xlab("") +
  ylab("Proportion ASPA+ Cells / "~ mm^2) +
