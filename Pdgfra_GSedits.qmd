...
```{r}
wilcox.test(prop_Pdgfra ~ Genotype, data = animal_frame_ASPAPdgfra)
```

```{r}
wilcox.test(prop_ASPA ~ Genotype, data = animal_frame_ASPAPdgfra)
```
...
prop_plot_gt <- animal_frame_ASPAPdgfra %>%
  ...
  mutate(
    total = ASPA + Pdgfra,
    prop_ASPA = ASPA / total,
    prop_Pdgfra = Pdgfra / total
  ) %>%
  select(Genotype, prop_ASPA, prop_Pdgfra) %>%
  pivot_longer(
  ...
  theme_classic() +
  scale_fill_manual(values = c("#00FF00", "#FFA500")) +
  scale_color_manual(values = c("#00FF00", "#FFA500")) +
  labs(x = "", y = "Proportion", fill = "")

ggsave("gt_propplot.png", plot = prop_plot_gt, width = 6, height = 4, dpi = 300)
prop_plot_gt
```
...
prop_plot_animal <- animal_frame_ASPAPdgfra %>%
  mutate(
    total_AP = total_ASPA + total_Pdgfra,
    prop_ASPA = total_ASPA / total_AP,
    prop_Pdgfra = total_Pdgfra / total_AP
  ) %>%
  select(Animal, Genotype, prop_ASPA, prop_Pdgfra) %>%
  pivot_longer(
    cols = starts_with("prop_"),
    names_to = "CellType",
    values_to = "Proportion"
  ...
  scale_fill_manual(values = c("ASPA+" = "#00FF00", "PDGFRα+" = "#FFA500")) +
  theme_classic() +
  labs(x = "Animal", y = "Proportion", fill = "") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

ggsave("ani_propplot.png", plot = prop_plot_animal, width = 6, height = 4, dpi = 300)
prop_plot_animal
```
