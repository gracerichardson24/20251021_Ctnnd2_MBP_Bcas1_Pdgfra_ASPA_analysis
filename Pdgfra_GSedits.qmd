...
for (img in unique_images) {
  for (b in 1:10) {
    ...
    P_pos_A_pos_count <- sum(temp$Pdgfra_pos_ASPA_pos == "1") 
    P_pos_A_neg_count <- sum(temp$Pdgfra_pos_ASPA_neg == "1") 
    P_neg_A_pos_count <- sum(temp$Pdgfra_neg_ASPA_pos == "1") 
    P_neg_A_neg_count <- sum(temp$Pdgfra_neg_ASPA_neg == "1") 
    Olig2_count <- P_pos_A_pos_count + P_pos_A_neg_count + P_neg_A_pos_count + P_neg_A_neg_count
    
    P_pos_A_pos_den <- P_pos_A_pos_count / area_mm2
    P_pos_A_neg_den <- P_pos_A_neg_count / area_mm2
    P_neg_A_pos_den <- P_neg_A_pos_count / area_mm2
    P_neg_A_neg_den <- P_neg_A_neg_count / area_mm2
    Olig2_den <- Olig2_count / area_mm2
    ...
    combined_detections_ASPA_Pdgfrabinned_counts <- rbind(
      ...
      data.frame(
        ...
        P_pos_A_pos = P_pos_A_pos_den,
        P_pos_A_neg = P_pos_A_neg_den,
        P_neg_A_pos = P_neg_A_pos_den,
        P_neg_A_neg = P_neg_A_neg_den,
        Olig2 = Olig2_den
      )
    )
  }
...
# Remove any images with all zeros if desired 
imageframe_combined_detections_ASPAPdgra <- imageframe_combined_detections_ASPAPdgra %>% filter(!(Pdgfra_pos_ASPA_pos == 0 & Pdgfra_pos_ASPA_neg == 0 & Pdgfra_neg_ASPA_pos == 0 & Pdgfra_neg_ASPA_neg == 0))
...
animal_frame_ASPAPdgfra <- imageframe_combined_detections_ASPAPdgra %>%
  ...
  summarise(
    ...
    prop_Pdgfra = total_Pdgfra / total_Olig2,
    prop_ASPA = total_ASPA / total_Olig2,
...
```{r}
Olig2_binned = lm(Olig2 ~ bin * Genotype,
           data = combined_detections_ASPA_Pdgfrabinned_counts_animal)
summary(Olig2_binned)
Anova(Olig2_binned, type = "II") 
```

```{r}
library(plotrix)
#line plot for Olig2+counts
...
ggplot(combined_detections_ASPA_Pdgfrabinned_counts_animal_summary, aes(x = bin, y = mean_Olig2, color = Genotype, group = Genotype)) +
...
png(filename = "p30_Ctnnd2_lineplot_Olig2_binned.png", width = 1200, height = 850, res = 300)
...
png(
  filename = "p30_Ctnnd2_lineplot_P_neg_A_neg_counts_binned.png",
  width = 1200,
  height = 850,
  res = 300
)
...
# Load the image-level data


# Ensure the genotype columns are factors with the same levels
animal_frame_ASPAPdgfra$Genotype <- factor(animal_frame_ASPAPdgfra$Genotype, levels = c("WT", "KO"))
imageframe_combined_detections_ASPAPdgfra$Genotype <- factor(imageframe_combined_detections_ASPAPdgfra$Genotype, levels = c("WT", "KO"))

# Summarize the animal-level data
summary_stats <- animal_frame_ASPAPdgfra %>%
  group_by(Genotype) %>%
  summarise(
...
ASPA_plot <- ggplot(data = animal_frame_ASPAPdgfra, aes(x = Genotype, y = mean_ASPA_per_area)) +
  ...
  geom_jitter(data = animal_frame_ASPAPdgfra,
              aes(color = Genotype, y = mean_ASPA_per_area),
              width = 0.2, size = 2, alpha = 0.9) +
  geom_jitter(data = imageframe_combined_detections_ASPAPdgfra,
              aes(color = Genotype, y = ASPA_per_area),
              width = 0.2, size = 2, alpha = 0.2) +
  theme_minimal() +
  xlab("") +
  ylab(expression("Mean ASPA+ Cells / " * mm^2)) +
  theme(legend.position = "none") +
  scale_fill_manual(values = c("dodgerblue", "firebrick1")) +
  scale_color_manual(values = c("dodgerblue", "firebrick1"))

ggsave("ASPA_density.png", plot = ASPA_plot, width = 2, height = 4, dpi = 300)
...
animal_frame_ASPAPdgfra <- animal_frame_ASPAPdgfra %>%
  mutate(Genotype = factor(Genotype, levels = c("WT", "KO")))

animal_frame_ASPAPdgra <- animal_frame_ASPAPdgra %>%
  mutate(Genotype = factor(Genotype, levels = c("WT", "KO")))

imageframe_combined_detections_ASPAPdgfra <- imageframe_combined_detections_ASPAPdgfra %>%
  mutate(Genotype = factor(Genotype, levels = c("WT", "KO")))
...
# Ensure Genotype is a factor with defined levels in both datasets
animal_frame_ASPAPdgfra$Genotype <- factor(animal_frame_ASPAPdgfra$Genotype, levels = c("WT", "KO"))
imageframe_combined_detections_ASPAPdgfra$Genotype <- factor(imageframe_combined_detections_ASPAPdgfra$Genotype, levels = c("WT", "KO"))

# Compute group-level mean and SEM of mean_olig2_per_area by genotype
summary_stats <- animal_frame_ASPAPdgfra %>%
  group_by(Genotype) %>%
  summarise(
...
# Create Olig2+ cell density plot
Olig2_plot <- ggplot(data = animal_frame_ASPAPdgfra, aes(x = Genotype, y = mean_olig2_per_area)) +
  ...
  # Animal-level jitter points (solid)
  geom_jitter(aes(color = Genotype, y = mean_olig2_per_area),
              width = 0.2, size = 2, alpha = 0.9) +
  # Image-level jitter points (transparent)
  geom_jitter(data = imageframe_combined_detections_ASPAPdgfra,
              aes(color = Genotype, y = Olig2_per_area),
              width = 0.2, size = 2, alpha = 0.2) +
  theme_minimal() +
  xlab("") +
  ylab(expression("Mean Olig2+ Cells / " * mm^2)) +
  theme(legend.position = "none") +
  scale_fill_manual(values = c("dodgerblue", "firebrick1")) +
  scale_color_manual(values = c("dodgerblue", "firebrick1"))

# Save and show the plot
ggsave("Olig2_density.png", plot = Olig2_plot, width = 2, height = 4, dpi = 300)
Olig2_plot
```
...
# Compute per-animal PDGFRA proportion
df_prop <- animal_frame_ASPAPdgfra %>%
  mutate(
    Pdgfra_pos = mean_Pdgfra_per_area,
    Pdgfra_neg = mean_olig2_per_area - mean_Pdgfra_per_area,
    prop_Pdgfra = Pdgfra_pos / total_Olig2
  )

# Inspect per-animal proportions
print(df_prop %>% select(Animal, Genotype, prop_Pdgfra))

# Plot (quick check)
boxplot(prop_Pdgfra ~ Genotype, data = df_prop,
        ylab = "Proportion PDGFRA+ (of Olig2+)", xlab = "Genotype")

# Compare genotypes (small n -> Wilcoxon preferred)
wilcox.test(prop_Pdgfra ~ Genotype, data = df_prop, exact = TRUE)

# Optional parametric test
t.test(prop_Pdgfra ~ Genotype, data = df_prop)
...
animal_frame_ASPAPdgfra <- animal_frame_ASPAPdgfra |>
  mutate(
    prop_Pdgfrapos_ASPApos = total_Pdgfra_pos_ASPA_pos / total_Olig2,
    propPdgfrapos_ASPAneg = total_Pdgfra_pos_ASPA_neg / total_Olig2,
    propPdgfraneg_ASPApos = total_Pdgfra_neg_ASPA_pos / total_Olig2,
    propPdgfra_neg_ASPAneg = total_Pdgfra_neg_ASPA_neg / total_Olig2
  )
```

```{r}
wilcox.test(prop_Pdgfrapos_ASPApos ~ Genotype, data = animal_frame_ASPAPdgfra)
wilcox.test(propPdgfrapos_ASPAneg ~ Genotype, data = animal_frame_ASPAPdgfra)
wilcox.test(propPdgfraneg_ASPApos ~ Genotype, data = animal_frame_ASPAPdgfra)
wilcox.test(propPdgfra_neg_ASPAneg ~ Genotype, data = animal_frame_ASPAPdgfra)
```

```{r}
agg <- animal_frame_ASPAPdgfra %>%
  group_by(Genotype) %>%
  summarise(
    count = sum(total_Pdgfra_pos_ASPA_pos),
    total = sum(total_Olig2),
    .groups = "drop"
  ) %>%
  mutate(other = total - count)

mat <- as.matrix(agg[, c("count", "other")])
rownames(mat) <- agg$Genotype

chisq.test(mat)

agg2 <- animal_frame_ASPAPdgfra %>%
  group_by(Genotype) %>%
  summarise(
    count = sum(total_Pdgfra_pos_ASPA_neg),
    total = sum(total_Olig2),
    .groups = "drop"
  ) %>%
  mutate(other = total - count)

mat2 <- as.matrix(agg2[, c("count", "other")])
rownames(mat) <- agg2$Genotype

chisq.test(mat2)

agg3 <- animal_frame_ASPAPdgfra %>%
  group_by(Genotype) %>%
  summarise(
    count = sum(total_Pdgfra_neg_ASPA_pos),
    total = sum(total_Olig2),
    .groups = "drop"
  ) %>%
  mutate(other = total - count)

mat3 <- as.matrix(agg3[, c("count", "other")])
rownames(mat) <- agg3$Genotype

chisq.test(mat3)

agg4 <- animal_frame_ASPAPdgfra %>%
  group_by(Genotype) %>%
  summarise(
    count = sum(total_Pdgfra_neg_ASPA_neg),
    total = sum(total_Olig2),
    .groups = "drop"
  ) %>%
  mutate(other = total - count)

mat4 <- as.matrix(agg4[, c("count", "other")])
rownames(mat) <- agg4$Genotype
...
prop_plot_gt <- animal_frame_ASPAPdgfra %>%
  group_by(Genotype) %>%
  summarise(
    Olig2  = sum(total_Olig2, na.rm = TRUE),
    ASPA   = sum(total_ASPA, na.rm = TRUE),
    Pdgfra = sum(total_Pdgfra, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  mutate(
    Other = Olig2 - ASPA - Pdgfra,
    prop_ASPA = ASPA / Olig2,
    prop_Pdgfra = Pdgfra / Olig2,
    prop_Other = Other / Olig2
  ) %>%
  select(Genotype, prop_ASPA, prop_Pdgfra, prop_Other) %>%
  pivot_longer(
  ...
  mutate(
    CellType = case_when(
      CellType == "prop_ASPA" ~ "ASPA+",
      CellType == "prop_Pdgfra" ~ "PDGFRα+",
      CellType == "prop_Other" ~ "Other Olig2+"
    ),
    CellType = factor(
      CellType,
      levels = c("ASPA+", "PDGFRα+", "Other Olig2+")
    )
  ) %>%
...
prop_plot_animal <- animal_frame_ASPAPdgfra %>%
  mutate(
    Other = total_Olig2 - total_ASPA - total_Pdgfra,
    prop_ASPA = total_ASPA / total_Olig2,
    prop_Pdgfra = total_Pdgfra / total_Olig2,
    prop_Other = Other / total_Olig2
  ) %>%
  select(Animal, Genotype, prop_ASPA, prop_Pdgfra, prop_Other) %>%
  pivot_longer(
    cols = starts_with("prop_"),
    names_to = "CellType",
    values_to = "Proportion"
  ...
  ggplot(aes(x = Animal, y = Proportion, fill = CellType)) +
  geom_col(show.legend = FALSE) +
  facet_grid(CellType ~ Genotype, scales = "free_x", switch = "y") +
  scale_fill_manual(values = c(
    "ASPA+" = "#00FF00",
    "PDGFRα+" = "#FFA500",
    "Other Olig2+" = "grey70"
...
ggsave(
  "ani_propplot_olig2_faceted.png",
  plot = prop_plot_animal,
  width = 9,
  height = 4,
  dpi = 300
)
...
# Set Genotype as an ordered factor
animal_frame_ASPAPdgfra <- animal_frame_ASPAPdgfra %>%
  mutate(Genotype = factor(trimws(Genotype), levels = c("WT", "KO")))

imageframe_combined_detections_ASPAPdgfra <- imageframe_combined_detections_ASPAPdgfra %>%
  mutate(Genotype = factor(trimws(Genotype), levels = c("WT", "KO")))

# Remove rows with zero values for this measure
animal_frame_ASPAPdgfra_filtered <- animal_frame_ASPAPdgfra %>%
  filter(mean_Pdgfra_neg_ASPA_neg_per_area != 0)

imageframe_combined_detections_ASPAPdgfra_filtered <- imageframe_combined_detections_ASPAPdgfra %>%
  filter(Pdgfra_neg_ASPA_neg_per_area != 0)
...
# Compute group mean and SEM after filtering out zeros
summary_stats <- animal_frame_ASPAPdgfra_filtered %>%
  group_by(Genotype) %>%
  summarise(
    MeanCount = mean(mean_Pdgfra_neg_ASPA_neg_per_area, na.rm = TRUE),
    SEM = sd(mean_Pdgfra_neg_ASPA_neg_per_area, na.rm = TRUE) /
      sqrt(sum(!is.na(mean_Pdgfra_neg_ASPA_neg_per_area))),
    .groups = "drop"
  )

# Create plot
Doubleneg_plot <- ggplot(
  data = animal_frame_ASPAPdgfra_filtered,
  aes(x = Genotype, y = mean_Pdgfra_neg_ASPA_neg_per_area)
) +
  geom_col(
    data = summary_stats,
  ...
  geom_jitter(
    data = imageframe_combined_detections_ASPAPdgfra_filtered,
    aes(x = Genotype, y = Pdgfra_neg_ASPA_neg_per_area, color = Genotype),
    width = 0.2,
    size = 2,
    alpha = 0.2,
    inherit.aes = FALSE
  ...
  theme_minimal() +
  xlab("") +
  ylab(expression("Mean Double- Cells / " * mm^2)) +
  theme(legend.position = "none") +
