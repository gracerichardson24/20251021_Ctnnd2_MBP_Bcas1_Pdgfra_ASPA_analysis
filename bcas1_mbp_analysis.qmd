...
imageframe_BCas1MBP <- Detections_data_height %>%
  ...
  mutate(
    ...
    total_Bcas1posMBPneg = total_Bcas1 - total_Bcas1MBP,
    Bcas1posMBPneg_per_area = total_Bcas1posMBPneg / Area,
...
animal_frame_BCas1MBP <- imageframe_BCas1MBP %>%
  ...
  summarise(
    ...
    total_Bcas1posMBPneg_count = sum(total_Bcas1posMBPneg, na.rm = TRUE),
    ...
    mean_Bcas1posMBPneg = mean(total_Bcas1posMBPneg, na.rm = TRUE),
    mean_Bcas1posMBPneg_per_area = mean(Bcas1posMBPneg_per_area, na.rm = TRUE),
  ...
  mutate(
    ...
    Bcas1posMBPneg_per_area = total_Bcas1posMBPneg_count / total_area
  )
```
...
animal_frame_BCas1MBP_prop <- animal_frame_BCas1MBP |>
  mutate(
    prop_myelinating = total_Bcas1MBP_count / total_Olig2,
    prop_unmyelinating = total_Bcas1posMBPneg_count / total_Olig2
  )

MBPprop_plot_gt <- animal_frame_BCas1MBP_prop %>%
  group_by(Genotype) %>%
  summarise(
    Bcas1_MBPneg = sum(total_Bcas1posMBPneg_count, na.rm = TRUE),
    Bcas1_MBPpos = sum(total_Bcas1MBP_count, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  mutate(
    total = Bcas1_MBPneg + Bcas1_MBPpos,
    prop_Bcas1_MBPneg = Bcas1_MBPneg / total,
    prop_Bcas1_MBPpos = Bcas1_MBPpos / total
  ) %>%
  select(Genotype, prop_Bcas1_MBPneg, prop_Bcas1_MBPpos) %>%
  pivot_longer(
  ...
  mutate(
    CellType = case_when(
      CellType == "prop_Bcas1_MBPneg" ~ "Bcas1+/MBP-",
      CellType == "prop_Bcas1_MBPpos" ~ "Bcas1+/MBP+"
    ),
    CellType = factor(CellType, levels = c("Bcas1+/MBP-", "Bcas1+/MBP+"))
  ) %>%
...
ggsave(
  "MBPgt_propplot_faceted.png",
  plot = MBPprop_plot_gt,
  width = 6,
  height = 3,
  dpi = 300
)
...
MBPprop_plot_animal <- animal_frame_BCas1MBP %>%
  mutate(
    total_BM = total_Bcas1posMBPneg_count + total_Bcas1MBP_count,
    prop_Bcas1_MBPneg = total_Bcas1posMBPneg_count / total_BM,
    prop_Bcas1_MBPpos = total_Bcas1MBP_count / total_BM
  ) %>%
  select(Animal, Genotype, prop_Bcas1_MBPneg, prop_Bcas1_MBPpos) %>%
  pivot_longer(
    cols = starts_with("prop_"),
    names_to = "CellType",
    values_to = "Proportion"
...
ggsave(
  "MBPani_propplot_grid.png",
  plot = MBPprop_plot_animal,
  width = 6,
  height = 4,
  dpi = 300
)
...
```{r}
animal_frame_BCas1MBP <- animal_frame_BCas1MBP |>
  mutate(
    propBcas1posMBPpos = total_Bcas1MBP_count /total_Bcas1_count,
    propBcas1posMBPneg = total_Bcas1posMBPneg_count / total_Bcas1_count
  )
wilcox.test(propBcas1posMBPpos ~ Genotype, data = animal_frame_BCas1MBP)
wilcox.test(propBcas1posMBPneg ~ Genotype, data = animal_frame_BCas1MBP)
```

```{r}
agg5 <- animal_frame_BCas1MBP %>%
  group_by(Genotype) %>%
  summarise(
    count = sum(total_Bcas1MBP_count),
    total = sum(total_Bcas1_count),
    .groups = "drop"
  ) %>%
  mutate(other = total - count)

mat5 <- as.matrix(agg5[, c("count", "other")])
rownames(mat5) <- agg5$Genotype

chisq.test(mat5)

agg6 <- animal_frame_BCas1MBP %>%
  group_by(Genotype) %>%
  summarise(
    count = sum(total_Bcas1posMBPneg_count),
    total = sum(total_Bcas1_count),
    .groups = "drop"
  ) %>%
  mutate(other = total - count)

mat6 <- as.matrix(agg6[, c("count", "other")])
rownames(mat6) <- agg6$Genotype
